...
const MIN_POOL_AGE_MS = Math.max(5 * 60_000, Number(process.env.LIVE_MIN_POOL_AGE_MS) || 30 * 60_000);
const MIN_LIQUIDITY_USD = Math.max(25_000, Number(process.env.LIVE_MIN_LIQUIDITY_USD) || 50_000);
const MIN_ROUND_TRIP_RECOVERY_PCT = Math.min(99, Math.max(70, Number(process.env.LIVE_MIN_ROUND_TRIP_RECOVERY_PCT) || 90));
const MAX_BUY_PRICE_IMPACT_PCT = Math.min(20, Math.max(0.1, Number(process.env.LIVE_MAX_BUY_PRICE_IMPACT_PCT) || 3));
const MAX_SELL_PRICE_IMPACT_PCT = Math.min(30, Math.max(0.1, Number(process.env.LIVE_MAX_SELL_PRICE_IMPACT_PCT) || 5));
const MAX_TOP_HOLDER_PCT = Math.min(100, Math.max(1, Number(process.env.LIVE_MAX_TOP_HOLDER_PCT) || 20));
const MAX_TOP5_HOLDER_PCT = Math.min(100, Math.max(5, Number(process.env.LIVE_MAX_TOP5_HOLDER_PCT) || 60));
...
function reject(reason: string, details: Record<string, unknown>): LiveEntrySafetyResult {
...
export async function evaluateLiveEntrySafety(input: {
  ...
  try {
    ...
    if (top1Pct > MAX_TOP_HOLDER_PCT) return reject("top_holder_concentration", details);
    if (top5Pct > MAX_TOP5_HOLDER_PCT) return reject("top5_holder_concentration", details);

    const pairs = await fetchJson(`${DEX_URL}/${encodeURIComponent(input.mint)}`);
    const candidates = (Array.isArray(pairs) ? pairs : []).filter(
      (pair: any) => pair?.chainId === "solana" && pair?.baseToken?.address === input.mint
    );
