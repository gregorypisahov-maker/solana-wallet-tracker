...
const MIN_LOCK_REMAINING_MS = Math.max(60_000, Number(process.env.ONCHAIN_LP_MIN_LOCK_REMAINING_MS) || 7 * 24 * 60 * 60_000);
...
export type OnchainLpResult = {
  verdict: OnchainLpVerdict;
  method: string;
  pool: string;
  lpMint: string | null;
  pctSafe: number | null;
...
function lockerAllowlist(): Set<string> {
  return new Set((process.env.ONCHAIN_LP_LOCKER_ALLOWLIST ?? "").split(",").map((v) => v.trim()).filter(Boolean));
}

function extensionType(item: any): string {
  return String(item?.extension ?? item?.extensionType ?? item?.type ?? "").toLowerCase();
}
...
export async function resolveTokenControls(mintAddress: string): Promise<TokenControlResult> {
  ...
  if (!cached.error && cached.data) {
    return { safe: cached.data.safe, reason: cached.data.reason, tokenProgram: cached.data.token_program, details: cached.data.details ?? {} };
  }
  ...
  if (info.mintAuthority) reason = "mint_authority_active";
    ...
    for (const ext of extensions) {
      ...
      if (type.includes("transferfeeconfig")) {
        const newer = state?.newerTransferFee ?? state?.newer_transfer_fee ?? {};
        const older = state?.olderTransferFee ?? state?.older_transfer_fee ?? {};
      ...
      if (type.includes("metadata") && (state?.updateAuthority || state?.update_authority)) details.mutableMetadataAuthority = state?.updateAuthority ?? state?.update_authority;
    }
  }
...
async function raydiumPoolMetadata(pool: string): Promise<any | null> {
  ...
  try {
    const response = await fetch(`https://api-v3.raydium.io/pools/info/ids?ids=${encodeURIComponent(pool)}`, { signal: controller.signal, headers: { Accept: "application/json" } });
    if (!response.ok) return null;
    const body = await response.json();
    return body?.data?.[0] ?? body?.data?.data?.[0] ?? null;
...
export async function resolveOnchainLpSafety(input: { mint: string; pool: string; dexId?: string | null }): Promise<OnchainLpResult> {
  ...
  if (!cached.error && cached.data && (!cached.data.expires_at || Date.parse(cached.data.expires_at) > Date.now())) {
    return { verdict: cached.data.verdict, method: cached.data.method, pool: input.pool, lpMint: cached.data.lp_mint, pctSafe: cached.data.pct_safe == null ? null : n(cached.data.pct_safe), unlockAt: cached.data.unlock_at, excludedAccounts: cached.data.excluded_accounts ?? [], details: cached.data.details ?? {} };
  }

  const connection = getLiveConnection();
  const poolKey = new PublicKey(input.pool);
  const poolInfo = await connection.getAccountInfo(poolKey, "confirmed");
  const poolProgram = poolInfo?.owner.toBase58() ?? null;
  const dex = String(input.dexId ?? "").toLowerCase();
  let verdict: OnchainLpVerdict = "UNKNOWN";
  let method = "onchain_pool_unresolved";
  let lpMint: string | null = null;
  let pctSafe: number | null = null;
  let unlockAt: string | null = null;
  const excludedAccounts = new Set<string>([input.pool]);
  const details: Record<string, unknown> = { version: ONCHAIN_RUG_SAFETY_VERSION, dexId: input.dexId ?? null, poolProgram };

  if ((dex === "pumpfun" || dex === "pump.fun") && poolProgram === PUMP_PROGRAM) {
    verdict = "CURVE";
    method = "pump_bonding_curve_program_owner";
  } else {
    ...
    for (const value of [ray?.vaultA, ray?.vaultB, ray?.baseVault, ray?.quoteVault]) {
      const address = typeof value === "string" ? value : value?.address;
      if (address) excludedAccounts.add(String(address));
    }
    details.raydiumApiResolved = Boolean(ray);

    if (lpMint) {
      const lpKey = new PublicKey(lpMint);
      const supplyResponse = await connection.getTokenSupply(lpKey, "confirmed");
      const supply = BigInt(supplyResponse.value.amount);
      const largest = await connection.getTokenLargestAccounts(lpKey, "confirmed");
      const lockers = lockerAllowlist();
      let safeAmount = 0n;
      let eoaAmount = 0n;
      const holders: any[] = [];
      for (const item of largest.value) {
        const account = item.address.toBase58();
        const amount = BigInt(item.amount);
        const accountInfo = await connection.getParsedAccountInfo(item.address, "confirmed");
        const owner = String((accountInfo.value?.data as any)?.parsed?.info?.owner ?? "");
        const accountProgram = accountInfo.value?.owner.toBase58() ?? null;
        const burn = BURN_ADDRESSES.has(account) || BURN_ADDRESSES.has(owner);
        const locker = lockers.has(owner) || (accountProgram ? lockers.has(accountProgram) : false);
        if (burn || locker) safeAmount += amount; else eoaAmount += amount;
        holders.push({ account, owner, accountProgram, amount: amount.toString(), burn, locker });
        if (burn || locker) excludedAccounts.add(account);
      }
      pctSafe = supply > 0n ? Number((safeAmount * 10_000n) / supply) / 100 : 100;
      details.lpSupply = supply.toString();
      details.lpHolders = holders;
  ...
  if (verdict === "LOCKED" && unlockAt && Date.parse(unlockAt) - Date.now() < MIN_LOCK_REMAINING_MS) { verdict = "UNLOCKED"; method = "lp_lock_too_close_to_unlock"; }
  const result: OnchainLpResult = { verdict, method, pool: input.pool, lpMint, pctSafe, unlockAt, excludedAccounts: [...excludedAccounts], details };
  const ttl = verdict === "UNKNOWN" ? CACHE_UNKNOWN_MS : CACHE_STATIC_MS;
  await supabase.from("lp_onchain_resolutions").upsert({ pool: input.pool, mint: input.mint, dex_id: input.dexId ?? null, pool_program: poolProgram, lp_mint: lpMint, verdict, method, pct_safe: pctSafe, unlock_at: unlockAt, excluded_accounts: result.excludedAccounts, details, resolved_at: new Date().toISOString(), expires_at: new Date(Date.now() + ttl).toISOString() });
  return result;
}
