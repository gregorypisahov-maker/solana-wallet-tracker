...
async function notifyTelegram(message: string): Promise<void> {
  try { await sendTelegramAlert(message); }
  catch (error) { console.error("[live-executor] Telegram notification failed", error); }
}
async function heartbeat(reason?: string): Promise<void> {
  await supabase.from("live_executor_state").update({ last_heartbeat_at: new Date().toISOString(), ...(reason ? { halt_reason: reason } : {}), updated_at: new Date().toISOString() }).eq("id", 1);
}
async function loadState(): Promise<ExecutorState> {
  ...
  if (state.daily_date !== today) {
    const { data: reset, error: resetError } = await supabase.from("live_executor_state").update({ daily_date: today, daily_entries: 0, daily_realized_pnl_sol: 0, updated_at: new Date().toISOString() }).eq("id", 1).select("*").single();
...
async function reject(signal: Signal, reason: string): Promise<void> {
  const { error } = await supabase.from("live_trade_signals").update({ status: "rejected", rejection_reason: reason, completed_at: new Date().toISOString() }).eq("id", signal.id).eq("status", "pending");
  if (error) throw new Error(error.message);
...
async function createSourceSignals(state: ExecutorState): Promise<void> {
  ...
  if (state.daily_entries < state.max_daily_entries && await openPositionCount() < state.max_open_positions) {
    ...
    if (source) await supabase.from("live_trade_signals").upsert({ id: randomUUID(), strategy: APPROVED_STRATEGY, source_position_id: source.position_id, mint: source.mint, token_symbol: source.token_symbol, side: "buy", requested_size_sol: Math.min(n(source.size_sol), n(state.max_position_sol)), max_slippage_bps: Math.min(200, Math.max(10, Number(process.env.LIVE_MAX_SLIPPAGE_BPS) || 100)), metadata: { source: "ai_discovery_positions", source_opened_at: source.opened_at } }, { onConflict: "strategy,source_position_id,side", ignoreDuplicates: true });
  }
  ...
  for (const position of (positions ?? []) as LivePosition[]) {
    ...
    if (!close) continue;
    await supabase.from("live_trade_signals").upsert({ id: randomUUID(), strategy: APPROVED_STRATEGY, source_position_id: position.source_position_id, mint: position.mint, token_symbol: position.token_symbol, side: "sell", requested_token_amount: position.token_amount, max_slippage_bps: Math.min(200, Math.max(10, Number(process.env.LIVE_MAX_SLIPPAGE_BPS) || 100)), metadata: { source: "ai_discovery_trades", exit_reason: close.exit_reason, source_closed_at: close.closed_at } }, { onConflict: "strategy,source_position_id,side", ignoreDuplicates: true });
  }
}
...
async function executeBuy(signal: Signal, state: ExecutorState): Promise<void> {
  ...
  try {
    ...
    await notifyTelegram(["🟢 <b>REAL MONEY TRADE OPENED</b>", "", `Token: <b>${tokenName(signal)}</b>`, `Size: <b>${spentSol.toFixed(6)} SOL</b>`, `Wallet balance: <b>${(solAfter / LAMPORTS_PER_SOL).toFixed(6)} SOL</b>`, `Slippage limit: <b>${signal.max_slippage_bps / 100}%</b>`, "", `<a href="${txLink(result.signature)}">View confirmed transaction</a>`, `<a href="https://dexscreener.com/solana/${encodeURIComponent(signal.mint)}">Open token chart</a>`].join("\n"));
  } catch (cause) {
    ...
    await notifyTelegram(["⚠️ <b>REAL MONEY BUY FAILED</b>", "", `Token: <b>${tokenName(signal)}</b>`, `Requested: <b>${sizeSol.toFixed(6)} SOL</b>`, `Reason: <code>${escapeHtml(reason)}</code>`, "", "🛑 The real executor was halted automatically. No confirmed live position was recorded."].join("\n"));
    throw cause;
  }
}
async function executeSell(signal: Signal, state: ExecutorState): Promise<void> {
  ...
  try {
    ...
    const quote = result.quote as Record<string, unknown>;
    await supabase.from("live_orders").update({ status: "submitted", tx_signature: result.signature, quoted_input_amount: String(quote.inAmount ?? ""), quoted_output_amount: String(quote.outAmount ?? ""), quote, updated_at: new Date().toISOString() }).eq("id", orderId);
    ...
    const receivedLamports = walletDeltaLamports > 0 ? walletDeltaLamports : quotedOutputLamports;
    if (sold <= 0n || !Number.isFinite(receivedLamports) || receivedLamports <= 0) throw new Error("sell_reconciliation_balance_delta_invalid");
    ...
    const now = new Date().toISOString();
    await supabase.from("live_orders").update({ status: "confirmed", tx_signature: result.signature, quoted_input_amount: String(quote.inAmount ?? ""), quoted_output_amount: String(quote.outAmount ?? ""), actual_input_amount: sold.toString(), actual_output_amount: String(receivedLamports), quote, error: null, updated_at: now }).eq("id", orderId);
    ...
    await supabase.from("live_trade_signals").update({ status: "executed", rejection_reason: null, completed_at: now }).eq("id", signal.id).eq("status", "claimed");
    ...
    await notifyTelegram([pnlSol >= 0 ? "🟢 <b>REAL MONEY TRADE CLOSED — PROFIT</b>" : "🔴 <b>REAL MONEY TRADE CLOSED — LOSS</b>", "", `Token: <b>${tokenName(signal)}</b>`, `Exit: <b>${escapeHtml(exitReason(signal))}</b>`, `Spent: <b>${spentSol.toFixed(6)} SOL</b>`, `Returned: <b>${proceedsSol.toFixed(6)} SOL</b>`, `Net PnL: <b>${pnlSol >= 0 ? "+" : ""}${pnlSol.toFixed(6)} SOL (${pnlSol >= 0 ? "+" : ""}${pnlPct.toFixed(2)}%)</b>`, `Wallet balance: <b>${(solAfter / LAMPORTS_PER_SOL).toFixed(6)} SOL</b>`, "", `<a href="${txLink(result.signature)}">View confirmed transaction</a>`].join("\n"));
  } catch (cause) {
    const reason = cause instanceof Error ? cause.message : "unknown_execution_error";
    const now = new Date().toISOString();
    await supabase.from("live_orders").update({ status: "failed", error: reason, updated_at: now }).eq("id", orderId);
    ...
    await notifyTelegram(["🚨 <b>REAL MONEY SELL FAILED</b>", "", `Token: <b>${tokenName(signal)}</b>`, `Reason: <code>${escapeHtml(reason)}</code>`, "", "🛑 The executor was halted and the position requires reconciliation. Check the wallet and dashboard before resuming."].join("\n"));
    throw cause;
  }
}
...
export function startLiveExecutor(): void {
  ...
  void notifyTelegram(["✅ <b>REAL MONEY TELEGRAM ALERTS ACTIVE</b>", "", "The live executor is online. You will receive alerts for confirmed buys, confirmed sells with exact PnL, and failed live orders.", "", "This is a system-status message — no trade was executed."].join("\n"));
