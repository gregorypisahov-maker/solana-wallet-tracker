...
function windowStats(trades: Row[], fromMs: number, toMs = Date.now()) {
  ...
  return {
    ...
    losses: filtered.filter((trade) => Number(trade.pnl ?? trade.pnl_sol) < 0).length,
    pnlSol,
  };
}
...
export async function GET(request: NextRequest) {
  ...
  const bots = [
    ...
    ...bot,
    recent24h: windowStats(bot.recentTrades, now - h24),
    recent48h: windowStats(bot.recentTrades, now - h48),
    previous48h: windowStats(bot.recentTrades, now - h48 * 2, now - h48),
  }));
