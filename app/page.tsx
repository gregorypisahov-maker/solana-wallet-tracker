...
function BotMark({ id }: { id: Bot["id"] }) { return <div className={`botMark ${id}`} aria-hidden="true"><span>{id === "legion" ? "L" : id === "scalper" ? "S" : "Ø"}</span></div>; }
...
export default function Dashboard() {
  ...
  if (selected) {
    ...
    const canResume = state.className !== "active";
    return <main className="cubeApp"><div className="detailShell"><button className="backButton" onClick={() => setSelectedBot(null)}>← Back</button>{notice && <div className="controlNotice">{notice}</div>}<section className={`detailHero ${selected.id}`}><BotMark id={selected.id} /><div><div className="botTitleRow"><h1>{selected.name}</h1><span className={`statusPill ${state.className}`}>{state.text}</span></div><p>{selected.subtitle}</p><strong className={selected.totalPnlSol >= 0 ? "positive" : "negative"}>{sol(selected.totalPnlSol)}</strong><div className="botControls"><button className="primaryControl" disabled={busyBot === selected.id || !canResume} onClick={() => controlBot(selected, "resume")}>{busyBot === selected.id ? "Updating…" : canResume ? "Resume bot" : "Bot running"}</button><button className="secondaryControl" disabled={busyBot === selected.id || canResume} onClick={() => controlBot(selected, "pause")}>Pause</button></div></div></section><section className="detailStats"><Metric label="Win rate" value={pct(selected.winRate)} sub={`${selected.wins}W / ${selected.losses}L`} /><Metric label="Profit factor" value={selected.profitFactor == null ? "—" : selected.profitFactor.toFixed(2)} /><Metric label="Completed trades" value={String(selected.completedTrades)} /><Metric label="Open positions" value={String(selected.openPositions)} /></section><section className="operationalGrid"><div><span>Status</span><b>{state.text}</b><small>{selected.state?.halt_reason ?? (state.className === "active" ? "Running normally" : "Disabled")}</small></div><div><span>Strategy</span><b>{selected.subtitle}</b><small>Paper trading only</small></div><div><span>Risk control</span><b>{selected.state?.halted ? "Triggered" : "Clear"}</b><small>{selected.state?.consecutive_losses ?? 0} consecutive losses</small></div></section><section className="activityPanel"><div className="sectionHead"><h2>Recent trades</h2><span>Latest activity</span></div>{selected.recentTrades.length ? selected.recentTrades.map((trade, index) => <ActivityRow key={`${trade.id ?? trade.position_id ?? index}`} trade={{ ...trade, botId: selected.id, botName: selected.name }} />) : <div className="emptyState">No completed trades yet.</div>}</section></div></main>;
  }

  const overviewWinRate = data.overview.completedTrades ? data.overview.wins / data.overview.completedTrades : 0;
  return <main className="cubeApp"><div className="appShell"><aside className="sideRail"><div className="brand"><CubeMark /><div><strong>Solana Tracker</strong><span>David&apos;s Heart</span></div></div><nav><button className="selected">Overview</button><button onClick={() => document.getElementById("bots")?.scrollIntoView({ behavior: "smooth" })}>Bots</button><button onClick={() => document.getElementById("activity")?.scrollIntoView({ behavior: "smooth" })}>Activity</button></nav><div className="systemBox"><span><i /> Systems online</span><small>Updated {new Date(data.generatedAt).toLocaleTimeString()}</small></div></aside><div className="mainPanel"><header className="mainHeader"><div><span className="micro">LIVE DASHBOARD</span><h1>Overview</h1><p>Simple performance view across all trading strategies.</p></div><div className="liveBadge"><i /> Live</div></header>{error && <div className="notice error">{error}. Showing the last successful snapshot.</div>}{notice && <div className="controlNotice">{notice}</div>}<section className="overviewGrid"><Metric label="Total PnL" value={sol(data.overview.totalPnlSol)} tone={data.overview.totalPnlSol >= 0 ? "positive" : "negative"} /><Metric label="Win rate" value={pct(overviewWinRate)} sub={`${data.overview.wins}W / ${data.overview.losses}L`} /><Metric label="Completed trades" value={String(data.overview.completedTrades)} /><Metric label="Open positions" value={String(data.overview.openPositions)} /></section><section id="bots"><div className="sectionHead"><div><span className="micro">TRADING BOTS</span><h2>Your bots</h2></div><span>Tap a bot for details</span></div><div className="botStack">{data.bots.map((bot) => <BotCard key={bot.id} bot={bot} onOpen={() => setSelectedBot(bot.id)} onResume={() => controlBot(bot, "resume")} busy={busyBot === bot.id} />)}</div></section><section id="activity" className="activityPanel"><div className="sectionHead"><div><span className="micro">RECENT ACTIVITY</span><h2>Latest trades</h2></div><span>{data.recentActivity.length} shown</span></div>{data.recentActivity.length ? data.recentActivity.map((trade, index) => <ActivityRow key={`${trade.botId}-${trade.id ?? trade.position_id ?? index}`} trade={trade} />) : <div className="emptyState">Waiting for completed trades.</div>}</section><footer>View-only paper trading dashboard. No wallet execution.</footer></div></div></main>;
}

function Metric({ label, value, sub, tone }: { label: string; value: string; sub?: string; tone?: "positive" | "negative" }) { return <div className="metricCard"><span>{label}</span><strong className={tone}>{value}</strong>{sub && <small>{sub}</small>}<i className="metricLine" /></div>; }
function BotCard({ bot, onOpen, onResume, busy }: { bot: Bot; onOpen: () => void; onResume: () => void; busy: boolean }) {
  const botStatus = status(bot); const needsResume = botStatus.className !== "active";
  return <div className={`botCard ${bot.id}`}><button className="botCardMain" onClick={onOpen}><div className="botIdentity"><BotMark id={bot.id} /><div><span className={`statusPill ${botStatus.className}`}>{botStatus.text}</span><h3>{bot.name}</h3><p>{bot.subtitle}</p></div></div><div className="botPnl"><strong className={bot.totalPnlSol >= 0 ? "positive" : "negative"}>{sol(bot.totalPnlSol)}</strong><span>{bot.completedTrades} trades</span></div><div className="botStats"><div><span>Win rate</span><b>{pct(bot.winRate)}</b></div><div><span>Profit factor</span><b>{bot.profitFactor == null ? "—" : bot.profitFactor.toFixed(2)}</b></div><div><span>Open</span><b>{bot.openPositions}</b></div></div><div className="openArrow">→</div></button>{needsResume && <button className="quickResume" disabled={busy} onClick={onResume}>{busy ? "Updating…" : "Resume"}</button>}</div>;
}
function ActivityRow({ trade }: { trade: any }) { const pnl = Number(trade.pnl ?? trade.pnl_sol ?? 0); const symbol = trade.token_symbol ?? trade.symbol ?? "UNKNOWN"; return <div className="activityRow"><BotMark id={trade.botId} /><div><strong>{symbol}</strong><span>{trade.botName}</span></div><span className={`resultTag ${pnl >= 0 ? "win" : "loss"}`}>{pnl >= 0 ? "WIN" : "LOSS"}</span><b className={pnl >= 0 ? "positive" : "negative"}>{sol(pnl)}</b><time>{timeAgo(trade.happenedAt)}</time></div>; }
