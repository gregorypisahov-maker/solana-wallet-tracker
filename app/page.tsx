...
export default function Dashboard() {
  ...
  if (!data) {
    ...
    return <main className="platformLogin"><div className="loadingPanel professionalLoader"><div className="loaderMark"><span className="loaderRing"/><span className="loaderCore">S</span></div><div className="loaderCopy"><strong>Solana Tracker</strong><p>{error ?? "Establishing secure connection"}</p><div className="loaderProgress" aria-hidden="true"><span/></div><small>Synchronizing strategy data and system status</small></div></div></main>;
  }
...
function ScanList({ scans, now }: any){return <div className="scanList">{scans.length?scans.slice(0,8).map((scan:any)=><div key={scan.id}><span className={`scanDot ${scan.status}`}/><div><strong>{scan.message??scan.status}</strong><small>{scan.scanned_count??0} scanned · {scan.qualified_count??0} qualified</small></div><time>{ageText(scan.finished_at??scan.started_at,now)}</time></div>):<div className="emptyState">No scan records</div>}</div>}
