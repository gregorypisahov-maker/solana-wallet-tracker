...
const status = `app.get("/api/status", async (_req: Request, res: Response) => {
  ...
  const [{ data: trades }, { data: rows }, { count: rollbackCount }] = await Promise.all([
    ...
    supabase.from("single_market_bot_trades").select("id,pnl_usdc,pnl_pct,status").in("status", ["paper_closed", "closed"]).order("id", { ascending: true }),
  ...
  const bookedPnlUsdc = closed.reduce((sum: number, row: any) => sum + n(row.pnl_usdc), 0);
  ...
  const equityUsdc = n(state.cash_usdc) + openValueUsdc;
  const accountPnlUsdc = equityUsdc - n(state.starting_cash_usdc);
  ...
  res.json({
    ...
    performance: {
      ...
      netPnlUsdc: bookedPnlUsdc,
      accountPnlUsdc,
      reconciliationDifferenceUsdc: bookedPnlUsdc - n(state.realized_pnl_usdc),
...
'<div class="card"><div class="muted">Account PnL</div><div class="v '+(pnl<0?'bad':'on')+'">'+money(pnl)+'</div></div>'+
