...
declare
  ...
  anchor_at timestamp with time zone := coalesce(new.entry_ts, new.observed_at);
begin
  if new.price_5m_usd is distinct from old.price_5m_usd then
    if new.price_5m_at is null
       or abs(extract(epoch from (new.price_5m_at - (anchor_at + interval '5 minutes')))) > 90 then
  ...
  if new.price_15m_usd is distinct from old.price_15m_usd then
    if new.price_15m_at is null
       or abs(extract(epoch from (new.price_15m_at - (anchor_at + interval '15 minutes')))) > 120 then
  ...
  if new.price_30m_usd is distinct from old.price_30m_usd then
    if new.price_30m_at is null
       or abs(extract(epoch from (new.price_30m_at - (anchor_at + interval '30 minutes')))) > 180 then
  ...
  if new.price_45m_usd is distinct from old.price_45m_usd then
    if new.price_45m_at is null
       or abs(extract(epoch from (new.price_45m_at - (anchor_at + interval '45 minutes')))) > 180 then
...
comment on function public.guard_ai_candidate_outcome_horizons() is
  'Validates AI outcome timestamps against entry_ts for entered rows and observed_at for non-entry samples.';
