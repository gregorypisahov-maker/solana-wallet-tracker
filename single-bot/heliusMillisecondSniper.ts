...
import {
  ...
  legOverheadSol,
  routeFeeSummary,
} from "../paper-trader/liveCostSimulation";
...
const DETECTION_ALERTS_ENABLED = !["0", "false", "no", "off", ""].includes(
  String(process.env.SNIPER_TELEGRAM_DETECTION_ALERTS || "false").trim().toLowerCase()
);
...
function isRateLimitError(error: unknown) {
  return /jupiter_quote_http_429|rate limit exceeded/i.test(error instanceof Error ? error.message : String(error));
}
async function telegram(message: string) {
  try {
    await sendTelegramAlert(message, { forceOperational: true });
  } catch (error) {
    console.warn("[helius-sniper] Telegram alert failed", error);
  }
}
...
async function audit(kind: string, mint: string | null, message: string, selected = false) {
  ...
  await supabase.from("scalp_scan_runs").insert({
    ...
    status: "ok",
    scanned_count: 1,
    qualified_count: selected ? 1 : 0,
    top_symbol: mint ? mint.slice(0, 8) : null,
    top_mint: mint,
    top_score: null,
    selected_mint: selected ? mint : null,
    message: `${kind}:${message}`,
    top_snapshot: { source: "helius_websocket", kind },
    created_at: now,
  });
}
...
async function extractMint(connection: Connection, signature: string): Promise<string | null> {
  ...
  const before = new Set((tx.meta.preTokenBalances || []).map((balance) => balance.mint));
  return (tx.meta.postTokenBalances || []).find(
    (balance) => balance.mint !== JUPITER_SOL_MINT && !before.has(balance.mint)
  )?.mint || null;
}
...
async function pairFor(mint: string) {
  ...
  const pair = (Array.isArray(body) ? body : [])
    .filter((item) => item?.chainId === "solana" && item?.baseToken?.address === mint && Number(item?.priceUsd || 0) > 0)
...
async function attempt(item: Candidate) {
  ...
  const buy = await quote({
    ...
    rawTokenAmount: lamports(tradeInputSol),
  ...
  const sell = await quote({
    ...
    rawTokenAmount: conservativeTokens.toString(),
    slippageBps: SLIPPAGE_BPS,
  });
  if (!sell.route || sell.outLamports <= 0n) return false;
  if (
    String((sell.raw as any)?.inputMint || "") !== item.mint ||
    String((sell.raw as any)?.outputMint || "") !== JUPITER_SOL_MINT
  ...
  if (immediateRoundTripNetPct < MAX_IMMEDIATE_ROUND_TRIP_LOSS_PCT) {
    return reject(
      item,
      `round_trip_cost_pct=${immediateRoundTripNetPct.toFixed(3)};floor=${MAX_IMMEDIATE_ROUND_TRIP_LOSS_PCT.toFixed(3)}`
    );
  }
  ...
  const snapshot = {
    ...
    launch_source: item.source,
    detection_signature: item.signature,
    detected_at: new Date(item.detectedAt).toISOString(),
    detection_to_entry_ms: latency,
    attempts: item.attempts,
    capital_debited_sol: SIZE_SOL,
    swap_input_sol: tradeInputSol,
    token_raw_amount: conservativeTokens.toString(),
    slippage_bps: SLIPPAGE_BPS,
    pool_liquidity_usd: pair.liquidityUsd,
    min_pool_liquidity_usd: MIN_POOL_LIQUIDITY_USD,
    jupiter_buy: buy.raw,
    jupiter_sell_check: sell.raw,
    entry_costs: routeFeeSummary(buy.raw),
    exit_preview_costs: routeFeeSummary(sell.raw),
    ...
    simulation_policy: "Jupiter expected-fill quote plus modeled network, priority and Jito costs",
    paper_only: true,
  };
  ...
  const { error } = await supabase.rpc("open_paper_scalp", {
    ...
    p_mint: item.mint,
    p_token_symbol: pair.symbol,
    p_pair_address: pair.address,
    p_entry_price_usd: pair.priceUsd,
    p_entry_time: now,
    p_size_sol: SIZE_SOL,
    p_entry_snapshot: snapshot,
  });
  if (error) {
  ...
  await audit(
    "entered",
    item.mint,
    `latency_ms=${latency};attempts=${item.attempts};liquidity_usd=${pair.liquidityUsd.toFixed(0)};round_trip_net=${roundTripNetSol.toFixed(6)}`,
    true
  );
  await telegram(
    `✅ <b>PAPER SNIPER OPENED</b>\n\n🪙 <b>${pair.symbol}</b>\nSize: <b>${SIZE_SOL.toFixed(3)} SOL</b>\n` +
    ...
    `Expected immediate round trip: <b>${roundTripNetSol.toFixed(6)} SOL</b>\n` +
    `Estimated immediate cost: <b>${((1 - roundTripNetSol / SIZE_SOL) * 100).toFixed(2)}%</b>\n\n` +
    `Mint:\n<code>${item.mint}</code>`
  );
  console.log(
    `[helius-sniper] ENTER ${pair.symbol} ${item.mint} liquidityUsd=${pair.liquidityUsd.toFixed(0)} ` +
    `latency=${latency}ms netRoundTrip=${roundTripNetSol.toFixed(6)}`
  );
  return true;
}
...
function trimPendingForNewest() {
  ...
  const oldest = [...pending.values()].sort((left, right) => left.detectedAt - right.detectedAt)[0];
...
function subscribe(connection: Connection, program: PublicKey, source: string) {
  return connection.onLogs(program, async ({ signature, err, logs }) => {
    ...
    try {
      ...
      if (
        DETECTION_ALERTS_ENABLED &&
        Date.now() - lastDetectionAlertAt >= DETECTION_ALERT_MIN_MS
      ) {
        lastDetectionAlertAt = Date.now();
        void telegram(
          `🚀 <b>SNIPER POOL MIGRATION DETECTED</b>\n\nSource: <b>${source}</b>\n` +
          `Mint:\n<code>${mint}</code>\n\nLiquidity gate runs before Jupiter quoting; newest migrations are prioritized.`
        );
      }
      void processQueue();
    } catch (error) {
      console.warn(`[helius-sniper] parse failed ${signature}`, error);
    }
  }, "confirmed");
}
...
export function startHeliusMillisecondSniper() {
  ...
  Promise.all([
    subscribe(connection, PUMP_PROGRAM, "pump_create"),
    subscribe(connection, PUMPSWAP_PROGRAM, "pumpswap_pool"),
  ])
    .then((ids) => {
      subscriptionIds = ids;
      console.log(
        `[helius-sniper] ACTIVE subscriptions=${ids.join(",")} retry=${RETRY_MS}ms ` +
        `jupiterMinInterval=${JUPITER_MIN_INTERVAL_MS}ms maxPending=${MAX_PENDING} ttlMs=${TTL_MS} ` +
        `minPoolLiquidityUsd=${MIN_POOL_LIQUIDITY_USD} costFloorPct=${MAX_IMMEDIATE_ROUND_TRIP_LOSS_PCT} ` +
        `detectionTelegram=${DETECTION_ALERTS_ENABLED} liveCostSimulation=true queueSource=pumpswap_pool`
      );
      void telegram(
        `🟢 <b>HELIUS SNIPER ONLINE</b>\n\nSubscriptions: <b>${ids.length}</b>\n` +
        `Trade queue source: <b>pumpswap_pool migrations only</b>\n` +
        `Raw migration alerts: <b>${DETECTION_ALERTS_ENABLED ? "ON" : "OFF"}</b>\n` +
    ...
    .catch((error) => {
      ...
      void telegram(
        `🚨 <b>HELIUS SNIPER CONNECTION FAILED</b>\n\n${error instanceof Error ? error.message : String(error)}\n\n` +
