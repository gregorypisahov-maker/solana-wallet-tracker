...
async function json(url: string) {
  ...
  try {
    const r = await fetch(url, { cache: "no-store", signal: controller.signal, headers: { Accept: "application/json", "User-Agent": "solana-scalper-shadow/1.0" } });
...
async function config(): Promise<Config> {
  const { data, error } = await supabase.from("scalper_shadow_config").select("*").eq("id", 1).single();
...
async function candidates(): Promise<Candidate[]> {
  ...
  return rows.map((row: any) => {
    ...
    return {
      mint: strip(row?.relationships?.base_token?.data?.id),
      symbol: String(a.name ?? "UNKNOWN").split("/")[0].trim(),
      pairAddress: String(a.address ?? ""),
      priceUsd: n(a.base_token_price_usd, NaN), liquidityUsd: n(a.reserve_in_usd, NaN),
      marketCapUsd: n(a.market_cap_usd ?? a.fdv_usd, NaN),
      change5m: n(a.price_change_percentage?.m5, NaN), change15m: n(a.price_change_percentage?.m15, NaN),
      volume5m: n(a.volume_usd?.m5, NaN), buys5m: n(tx.buys, NaN), sells5m: n(tx.sells, NaN),
      buyers5m: n(tx.buyers, NaN), poolAgeMinutes: Number.isFinite(created) ? (Date.now() - created) / 60_000 : NaN,
    };
...
function passes(c: Candidate, r: Config) {
  ...
  return c.liquidityUsd >= r.min_liquidity_usd && c.marketCapUsd >= r.min_market_cap_usd &&
    c.marketCapUsd <= r.max_market_cap_usd && ratio >= r.min_liquidity_to_mcap &&
    c.change5m >= r.min_five_minute_change_pct && c.change5m <= r.max_five_minute_change_pct &&
    c.change15m >= r.min_fifteen_minute_change_pct && c.change15m <= r.max_fifteen_minute_change_pct &&
    c.volume5m >= r.min_volume_usd && c.buyers5m >= r.min_buyers && flow >= r.min_buy_sell_ratio &&
    c.poolAgeMinutes >= r.min_pool_age_minutes && c.poolAgeMinutes <= r.max_pool_age_minutes;
}
...
function score(c: Candidate, r: Config) {
  ...
  return Math.round(Math.min(100,
    Math.min(25, c.change5m / Math.max(1, r.max_five_minute_change_pct) * 25) +
    Math.min(20, c.change15m / Math.max(1, r.max_fifteen_minute_change_pct) * 20) +
    Math.min(20, c.volume5m / 25_000 * 20) + Math.min(15, c.buyers5m / 60 * 15) +
    Math.min(10, ratio / 0.5 * 10) + Math.min(10, flow / 2 * 10)));
}
...
async function dexPrice(mint: string, pairAddress: string) {
  ...
  const pair = pairs.find((p: any) => p.pairAddress === pairAddress) ?? pairs.sort((a: any,b: any)=>n(b?.liquidity?.usd)-n(a?.liquidity?.usd))[0];
...
async function scan() {
  ...
  try {
    ...
    const [{ data: positions }, list] = await Promise.all([
      supabase.from("scalper_shadow_positions").select("*"), candidates()
    ]);
    await supabase.from("scalper_shadow_state").update({ last_scan_at: new Date().toISOString(), updated_at: new Date().toISOString(), enabled: true }).eq("id", 1);
    if ((positions ?? []).length >= rules.max_concurrent_positions) return;
    const openMints = new Set((positions ?? []).map((p: any) => p.mint));
    const eligible = list.filter((c) => !openMints.has(c.mint) && passes(c, rules)).sort((a,b)=>score(b,rules)-score(a,rules));
    const pick = eligible[0]; if (!pick) return;
    await supabase.from("scalper_shadow_positions").insert({
...
async function check() {
  ...
  try {
    const rules = await config();
    const { data: positions } = await supabase.from("scalper_shadow_positions").select("*");
    for (const p of positions ?? []) {
      try {
        const price = await dexPrice(p.mint, p.pair_address);
        if (!Number.isFinite(price) || price <= 0) continue;
        const entry = n(p.entry_price_usd); const oldPeak = n(p.peak_price_usd, entry); const peak = Math.max(oldPeak, price);
        ...
        const bankroll = n(state?.bankroll_sol, 1) + pnl;
        await supabase.from("scalper_shadow_trades").insert({ position_id:p.position_id,mint:p.mint,token_symbol:p.token_symbol,entry_price_usd:entry,exit_price_usd:price,size_sol:p.size_sol,pnl_sol:pnl,net_return_pct:net,exit_reason:reason,opened_at:p.entry_time,closed_at:new Date().toISOString(),entry_snapshot:p.entry_snapshot,exit_snapshot:{price,peak,peakNet,strategyVersion:rules.strategy_version} });
        await supabase.from("scalper_shadow_positions").delete().eq("position_id",p.position_id);
        await supabase.from("scalper_shadow_state").update({ bankroll_sol: bankroll, updated_at:new Date().toISOString() }).eq("id",1);
...
export function startScalperShadowScheduler() {
  if (["0","false","off"].includes((process.env.ENABLE_SCALPER_SHADOW ?? "true").toLowerCase())) return;
  console.log("[scalper-shadow] independent forward test enabled");
