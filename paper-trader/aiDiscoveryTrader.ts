  ...
  const lines = data.map(
    (row: any, index: number) =>
      `${index + 1}. <b>${row.token_symbol}</b> — ${row.score}/100 · ${row.status}\n${(row.reasons ?? [])
        .slice(0, 2)
        .join(", ")}${row.risks?.length ? ` · risk: ${row.risks.slice(0, 1).join(", ")}` : ""}`,
  );
...
async function scanEntries(): Promise<void> {
  ...
  try {
    ...
    if (
      state.entries_today >= MAX_DAILY_ENTRIES ||
    ...
    for (const opportunity of data ?? []) {
      if ((opportunity.risks ?? []).length > 1 || (await cooledDown(opportunity.mint))) {
        continue;
      }
      ...
      try {
        ...
        if (!market) {
          console.warn(
            `[ai-discovery-trader] skipped ${opportunity.token_symbol}: bound pair unavailable on DexScreener`,
          );
          continue;
        }
      ...
      try {
        ...
        if (!market) {
          if (heldMs < MAX_HOLD_MS) {
            console.warn(
              `[ai-discovery-trader] position ${position.token_symbol} price unavailable; holding fail-closed`,
            );
            continue;
          }
          ...
          if (fallbackPrice <= 0 || entry <= 0) {
            console.warn(
              `[ai-discovery-trader] position ${position.token_symbol} overdue but has no valid fallback price`,
            );
            continue;
          }
          ...
          const fallbackMarket = {
            ...
            marketCapUsd: null,
            changeM5: null,
            priceSource: "last_valid_recorded_price",
            fallbackReason: "bound_pair_unavailable_after_max_hold",
          };
          const grossPct = (fallbackPrice / entry - 1) * 100;

          console.warn(
            `[ai-discovery-trader] position ${position.token_symbol} exceeded max hold with unavailable pair; closing at last valid price`,
          );
        ...
        else if (peakPct >= TRAIL_ARM_PCT && pullbackPct <= -TRAIL_DISTANCE_PCT) {
...
export function startAiDiscoveryTrader(): void {
  ...
  console.log(
    `[ai-discovery-trader] ${VERSION} enabled; paper-only; size ${FIXED_SIZE_SOL.toFixed(2)} SOL; score ${MIN_SCORE}+`,
  );
