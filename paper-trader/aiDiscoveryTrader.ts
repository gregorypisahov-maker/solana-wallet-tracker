...
async function pairFor(mint: string, pairAddress: string, minimumLiquidity = 0): Promise<Market | null> {
...
async function priceFor(mint: string, pairAddress: string): Promise<Market | null> { return pairFor(mint, pairAddress, 25_000); }
...
function baselineProbability(opportunity: any): number {
  ...
  const raw = 25 + (score - 70) * 1.2 + Math.log10(Math.max(liquidity, 1)) * 4 + clamp(momentum, -10, 10) * 1.1 + clamp((buyers - sells) / Math.max(buyers + sells, 1), -1, 1) * 12 + regimeBoost - riskCount * 12;
...
async function recordObservation(opportunity: any, assessment: { passed: boolean; reasons: string[] }): Promise<number | null> {
  ...
  const features = {
    score: n(opportunity.score), confidence: opportunity.confidence, status: opportunity.status, marketRegime: opportunity.market_regime,
    liquidityUsd: n(opportunity.liquidity_usd), marketCapUsd: n(opportunity.market_cap_usd), priceChangeM5: n(opportunity.price_change_m5),
    priceChangeH1: n(opportunity.price_change_h1), volumeM5Usd: n(opportunity.volume_m5_usd), volumeH1Usd: n(opportunity.volume_h1_usd),
    buysM5: n(opportunity.buys_m5), sellsM5: n(opportunity.sells_m5), buyersM5: n(opportunity.buyers_m5), poolAgeMinutes: n(opportunity.pool_age_minutes),
    reasons: opportunity.reasons ?? [], risks: opportunity.risks ?? []
  };
  const { data, error } = await supabase.from("ai_candidate_observations").insert({
    mint: opportunity.mint, token_symbol: opportunity.token_symbol, pair_address: opportunity.pair_address,
    source_last_seen_at: opportunity.last_seen_at, features, rules_passed: assessment.passed,
    decision: assessment.passed ? "observe" : "reject", rejection_reasons: assessment.reasons,
    baseline_probability: baselineProbability(opportunity), model_version: SHADOW_MODEL_VERSION
  }).select("id").single();
  if (error) throw new Error(error.message);
  return Number(data.id);
}

async function markObservationEntered(id: number | null, entryPriceUsd: number): Promise<void> {
  if (!id) return;
  await supabase.from("ai_candidate_observations").update({ entered: true, decision: "enter", entry_price_usd: entryPriceUsd, updated_at: new Date().toISOString() }).eq("id", id);
}

async function collectCandidateObservations(cutoff: string): Promise<any[]> {
  const { data, error } = await supabase.from("market_opportunities").select("*").gte("last_seen_at", cutoff).order("score", { ascending: false }).limit(30);
  if (error) throw new Error(error.message);
  for (const opportunity of data ?? []) {
    try { await recordObservation(opportunity, ruleAssessment(opportunity)); }
    catch (error) { console.warn(`[ai-discovery-trader] observation failed for ${opportunity.token_symbol}`, error); }
  }
  return data ?? [];
}
...
async function scanEntries(): Promise<void> {
  ...
  try {
    ...
    for (const opportunity of opportunities.filter((item: any) => ruleAssessment(item).passed)) {
      const observationId = await recordObservation(opportunity, ruleAssessment(opportunity));
      if (await cooledDown(opportunity.mint)) continue;
      try { const market = await priceFor(opportunity.mint, opportunity.pair_address); if (!market) continue; if (market.changeM5 < 0 || market.changeM5 > 15) continue; await openTrade(state, opportunity, market, observationId); break; } catch (error) { console.warn(`[ai-discovery-trader] candidate ${opportunity.token_symbol} skipped`, error); }
    }
...
async function trackCandidateOutcomes(): Promise<void> {
  ...
  try {
    ...
    for (const observation of data ?? []) {
      const ageMinutes = (Date.now() - Date.parse(observation.observed_at)) / 60_000;
      const due = OUTCOME_HORIZONS.filter((minutes) => ageMinutes >= minutes && observation[`price_${minutes}m_usd`] == null);
      if (!due.length) continue;
      try {
        const market = await pairFor(observation.mint, observation.pair_address, 0);
        if (!market) continue;
        const basePrice = n(observation.entry_price_usd ?? observation.features?.priceUsd ?? observation.features?.price_usd, 0) || n(observation.features?.priceUsd, 0);
        const effectiveBase = basePrice > 0 ? basePrice : n(observation.features?.signalSnapshot?.priceUsd, 0);
        const updates: Record<string, unknown> = { updated_at: new Date().toISOString() };
        for (const minutes of due) {
          updates[`price_${minutes}m_usd`] = market.priceUsd;
          updates[`return_${minutes}m_pct`] = effectiveBase > 0 ? Number((((market.priceUsd / effectiveBase) - 1) * 100).toFixed(4)) : null;
        }
        if (ageMinutes >= 45) updates.outcome_complete = true;
        await supabase.from("ai_candidate_observations").update(updates).eq("id", observation.id);
      } catch (error) { console.warn(`[ai-discovery-trader] outcome tracking failed for ${observation.token_symbol}`, error); }
    }
    ...
    for (const position of await loadPositions()) {
      try {
        ...
        if (!market) {
          ...
          await closeTrade(position, { priceUsd: fallbackPrice, source: "last_valid_price" }, "max_hold_price_unavailable", (fallbackPrice / entry - 1) * 100);
          continue;
        }
        const entry = n(position.entry_price_usd);
...
export function startAiDiscoveryTrader(): void {
  if (!enabled()) { console.log("[ai-discovery-trader] disabled by ENABLE_AI_DISCOVERY_TRADER"); return; }
  console.log(`[ai-discovery-trader] ${VERSION} enabled; paper-only; shadow dataset ${SHADOW_MODEL_VERSION}; no daily entry pause; size ${FIXED_SIZE_SOL.toFixed(2)} SOL; score ${MIN_SCORE}+`);
  void scanEntries().catch((error) => console.error("[ai-discovery-trader] initial scan failed", error));
  void managePositions().catch((error) => console.error("[ai-discovery-trader] initial position check failed", error));
  void trackCandidateOutcomes().catch((error) => console.error("[ai-discovery-trader] initial outcome tracking failed", error));
  setInterval(() => void scanEntries().catch((error) => console.error("[ai-discovery-trader] scan failed", error)), 60_000);
  setInterval(() => void managePositions().catch((error) => console.error("[ai-discovery-trader] position check failed", error)), 10_000);
  setInterval(() => void trackCandidateOutcomes().catch((error) => console.error("[ai-discovery-trader] outcome tracking failed", error)), 60_000);
}
