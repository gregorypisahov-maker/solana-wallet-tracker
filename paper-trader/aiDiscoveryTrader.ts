...
const VERSION = "ai_discovery_trader_v1_1_2026_07_24";
  ...
  try {
    ...
    for (const opportunity of data ?? []) {
      ...
      try {
        ...
        if (!market) { console.warn(`[ai-discovery-trader] skipped ${opportunity.token_symbol}: bound pair unavailable on DexScreener`); continue; }
        if (market.changeM5 < 0 || market.changeM5 > 15) continue;
        await openTrade(state, opportunity, market); break;
      } catch (error) { console.warn(`[ai-discovery-trader] candidate ${opportunity.token_symbol} skipped`, error); }
    }
...
async function managePositions(): Promise<void> { if (positionRunning) return; positionRunning = true; try { for (const position of await loadPositions()) { try { const market = await priceFor(position.mint, position.pair_address); if (!market) { console.warn(`[ai-discovery-trader] position ${position.token_symbol} price unavailable; holding fail-closed`); continue; } const entry = n(position.entry_price_usd); const grossPct = (market.priceUsd / entry - 1) * 100; const peak = Math.max(n(position.peak_price_usd), market.priceUsd); const peakPct = (peak / entry - 1) * 100; const pullbackPct = (market.priceUsd / peak - 1) * 100; const heldMs = Date.now() - Date.parse(position.opened_at); let reason: string | null = null; if (grossPct <= HARD_STOP_PCT) reason = "hard_stop"; else if (grossPct >= TAKE_PROFIT_PCT) reason = "take_profit"; else if (peakPct >= TRAIL_ARM_PCT && pullbackPct <= -TRAIL_DISTANCE_PCT) reason = "trailing_stop"; else if (heldMs >= MAX_HOLD_MS) reason = "max_hold"; if (reason) await closeTrade(position, market, reason, grossPct); else await supabase.from("ai_discovery_positions").update({ last_price_usd: market.priceUsd, peak_price_usd: peak, last_checked_at: new Date().toISOString(), updated_at: new Date().toISOString() }).eq("position_id", position.position_id); } catch (error) { console.warn(`[ai-discovery-trader] position ${position.token_symbol} check skipped`, error); } } } finally { positionRunning = false; } }
