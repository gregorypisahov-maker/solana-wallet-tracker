...
async function processRecentBuy(row: any): Promise<void> {
  const signalId = randomUUID();
  const seenAt = new Date().toISOString();
  const reasons: string[] = [];
  const snapshot: Record<string, unknown> = {
    strategy_version: "tiered_entry_shadow_v1",
  ...
  try {
    ...
    else if (trust < 65) reasons.push("entry_wallet_trust_below_65");
    ...
    if (marketCap == null || !Number.isFinite(marketCap) || marketCap <= 0) reasons.push("missing_data:market_cap");
    else if (marketCap > 200_000) reasons.push("market_cap_above_200000");
    if (liquidity == null || !Number.isFinite(liquidity)) reasons.push("missing_data:liquidity_usd");
