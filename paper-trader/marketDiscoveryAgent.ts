...
async function fetchJson(url: string, cacheTtlMs = 30_000): Promise<any> {
  return fetchJsonQueued(url, {
    ...
    headers: {
      ...
      "User-Agent": "solana-market-discovery-ai/2.0",
    },
  });
}
...
function parseDexPair(pair: any, requestedMint: string): Candidate | null {
  ...
  const mint = baseMint === requestedMint ? baseMint : quoteMint === requestedMint ? quoteMint : baseMint;
...
async function discover(): Promise<Candidate[]> {
  ...
  if (byMint.size === 0) throw new Error("Dexscreener token lookups returned no valid Solana pairs");
...
export async function runMarketDiscoveryScan(): Promise<void> {
  ...
  try {
    ...
    await supabase.from("market_discovery_runs").insert({ started_at: startedAt, finished_at: now, status: "error", message, snapshot: { version: VERSION, provider: "dexscreener" } });
  } finally {
